...
def calc_degree_days(self, PLOT, FIGURE):
    ...
    if self.met_distribution.lower() == "point":
        ...
        for element in range(self.ATTM_nrows * self.ATTM_ncols):
            ...
            if np.size(spline.roots()) == 0:
                ...
                for i in range(0, np.size(spline.roots())-2, 2):
                    ...
                    print 'Year: '+str(year)+'TDD: '+str(self.TDD[k,element])+'FDD: '+\
                      str(self.FDD[k,element])
        ...
        if FIGURE == 'TRUE':
            ...
            pl.title('Thawing Degree-Days - Year'+str(k))
            pl.colorbar(extend = 'neither', shrink = 0.92)
            pl.savefig('Thawing_Degree_Days_'+str(year)+'.jpg', format = 'jpg')
            TDD_plot.tofile('Thawing_Degree_Days_'+str(year)+'.bin')
            pl.close()
            #--------------------------------------------------------
            ...
            pl.title('Freezing Degree-Days -'+str(year))
            pl.colorbar(extend = 'neither', shrink = 0.92)
            pl.savefig('Freezing_Degree_Days_'+str(year)+'.jpg', format = 'jpg')
            TDD_plot.tofile('Freezing_Degree_Days_'+str(year)+'.bin')
            pl.close()
